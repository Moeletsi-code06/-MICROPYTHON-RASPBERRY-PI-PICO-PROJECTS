...
def servo_access():
    servo.duty_u16(unlocked)
    utime.sleep(2)             #time delay of 2 seconds before it changes states
    servo.duty_u16(locked)
    
#Following function appends the formatted log entry data to the created log_book
def logging_function():      #appending logs to created file 
    access = -1
    with open("accessLogs.txt","a") as file:#appends to access log text file
        #displays access logs according to the number of entries:
        #for access in range(access_entries):   
...
def maintenance_threshold(success):
    global successful_access             
    successful_access = success
    threshold= 5
    if successful_access == threshold:
        lcd.clear()
...
authorised_codes = ["101","110","010", "001"]
user_type = ["General Staff", "Academic Staff","Student", "Maintenance"]
result = "DENIED"
...
#     accessLog["LOG NO."].append(log_num)   #appends the log entry number
    ...
    if accessC in authorised_codes:       #Condition that checks the access_code received earlier is within the authorized access code list
        ...
        log_num = log_num + 1      #Counter for the log number in the log entries
        accessLog["LOG NO."].append(log_num)   #appends the log entry number
        #access_entries += 1    #Counts how many entries in each loop
